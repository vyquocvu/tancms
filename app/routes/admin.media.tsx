        ...
        <div className='p-4'>
          <div className='flex items-start justify-between'>
            <div className='min-w-0 flex-1'>
              <h3 className='text-sm font-medium truncate'>{media.name}</h3>
              <p className='text-xs text-muted-foreground mt-1'>
                {formatFileSize(media.size)} • {media.createdAt}
              </p>
              {media.altText && (
                <p className='text-xs text-muted-foreground mt-1 truncate'>Alt: {media.altText}</p>
              )}
            </div>
            <div className='flex space-x-1 ml-2'>
              ...
              <Button variant='ghost' size='sm' onClick={() => onDelete(media)}>
  ...
  const mediaActions: BulkAction[] = [
    {
      ...
      confirmationMessage:
        'Are you sure you want to delete these media files? This action cannot be undone.',
    },
  ]
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                        setSelectedType(e.target.value as 'all' | 'image' | 'video' | 'document')
                      }
