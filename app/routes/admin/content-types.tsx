    ...
    <Card>
      <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
        ...
        <div className='flex items-center space-x-2'>
          <Badge variant='secondary'>{contentType._count.entries} entries</Badge>
        </div>
      </CardHeader>
      <CardContent>
        <div className='space-y-4'>
          ...
          <div className='flex justify-between items-center pt-2 border-t'>
            ...
            <div className='flex space-x-2'>
              <Button size='sm' variant='outline' onClick={() => onManageEntries(contentType.id)}>
                <Database className='h-4 w-4 mr-1' />
                Entries
              </Button>
              <Button size='sm' variant='outline' onClick={() => onEdit(contentType.id)}>
                <Edit className='h-4 w-4 mr-1' />
                Edit
              </Button>
              <Button size='sm' variant='destructive' onClick={() => onDelete(contentType.id)}>
...
export default function ContentTypes() {
  const [contentTypes, setContentTypes] = useState<
    (ContentType & { _count: { entries: number } })[]
  >([])
                ...
                <TableBody>
                  {contentTypes.map(contentType => (
                    <TableRow key={contentType.id}>
                      ...
                      <TableCell>
                        <Badge variant='outline'>{contentType._count.entries}</Badge>
                      </TableCell>
